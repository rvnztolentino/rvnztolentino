          ...
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-6 md:gap-8 mb-16 w-full">
              {PORTFOLIO_IMAGES.map((src, index) => (
                <div
                  key={index}
                  className={`relative w-full md:w-1/3 aspect-[3/4] overflow-hidden grayscale hover:grayscale-0 transition-all duration-500 ease-out group ${index === 1 ? 'md:-translate-y-12 z-10' : 'z-0'
                    }`}
                >
                  <img
                    src={src}
            ...
            <button
              onClick={() => {
                setViewState('list');
                if (scrollToSection) scrollToSection('projects');
              }}
                ...
                >
                  <div>
                    <a
                      ...
                      className="flex justify-between items-start mb-6 w-full group/link"
                    >
                      <h3 className="font-display font-bold text-2xl tracking-tight text-white group-hover:text-blue-200 transition-colors">
                        {project.title}
                      </h3>
                      <ExternalLink className="md:block hidden w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 group-hover/link:text-white" />
                    </a>
